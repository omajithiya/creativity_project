...
class MainActivity2 : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun startListening() {
        ...
        intent.putExtra(RecognizerIntent.EXTRA_PREFER_OFFLINE, false) // Use cloud-based recognition
    ...
    private fun getCachedResponse(query: String): String? {
        return responseCache[query.lowercase(Locale.ROOT)]
    }

    private fun saveToCache(query: String, response: String) {
        responseCache[query.lowercase(Locale.ROOT)] = response
    }

    private fun getAIResponse(userMessage: String) {
        val cachedResponse = getCachedResponse(userMessage)
            ...
            override fun onResponse(call: Call, response: Response) {
                try {
                    ...
                    val chatResponse = jsonResponse.getJSONArray("choices")
                        ...
                        .getString("content")
                    saveToCache(userMessage, chatResponse)
                    runOnUiThread {
                        speak(chatResponse)
                        handler.postDelayed({ startListening() }, 2000) // Auto-restart listening
                    }
                } catch (e: JSONException) {
                    Log.e("AI_ERROR", "JSON Parsing Error: ${e.message}")
                    runOnUiThread { speak("Sorry, I couldn't process that.") }
                }
            }
        })
    }
